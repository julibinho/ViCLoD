...
log_file_line = {"total":1, "threshold":2, "analyzed":3, "removed":4, "unannotated_for_clustering":5, "unannotated_for_tree":6, "low_quality":7, "low_abundance":8, "cumulated_criteria":9, "only_unannotated_for_tree":10}
...
def total_number_of_reads(log_file):
	...
	"""
	reads_info = {"threshold": [0.005,"percentage"], "total":1, "analyzed":1, "removed":0, "unannotated_for_clustering":0, "unannotated_for_tree":0, "low_quality":0, "low_abundance":0, "cumulated_criteria":[0,0,0,0], "only_unannotated_for_tree":0}
	...
	if (os.path.isfile(log_file)) : 
		...
		threshold = ((lines[log_file_line["threshold"]-1].split("\n")[0]).split(": ")[1]).split("\t")
		for i in range(len(threshold)) :
			if i < len(reads_info["threshold"]):
				reads_info["threshold"][i] = threshold[i]
		
		criteria = (lines[log_file_line["cumulated_criteria"]-1].split("\n")[0]).split("\t")
		for i in range(len(criteria)) :
			if i < len(reads_info["cumulated_criteria"]):
...
def get_repertoire_informations(file_name, colors, reads, clonotypes):
	...
	input reads:			Dict()		key=criteria, value=number of sequences corresponding to that citeria
	input clonotypes:		Dict()		key=subclone id (MobiLLe), value=dict of clonotype sequence ids and their abundance
