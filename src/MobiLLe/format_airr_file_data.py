...
output_files = {"matching_seq_file": "_seq_name_matching.txt", "unique_seq_name_file": "_unique_seq_id_matching.txt", "unannotated_seq_file": "_unannotated_seq.tsv", "annotated_seq_file": "_airr_new_id.tsv"}
...
def save_stats_data(stats, analysis_name, total_seqs, min_read) :
	"""
...
def format_sequences_information(airr_file_name, columns, analysis_name, threshold, threshold_type, quality_filter) :
	...
	input threshold_type:	str	measure to which the minimum number of readings is given
	input quality_filter:	str	if False, sequences contaning N are kept for the analysis
	"""
	...
	for sequence in unique_sequences :
		...
		if filters['minimum reads'] :
			...
			stats[sequence_info['sequence_id']] = filters
	save_stats_data(stats, analysis_name, total_seq, minimum_read)
